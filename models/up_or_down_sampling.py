...
class Conv2d(nn.Module):
    def __init__(self, fmaps, kernel, up=False, down=False, resample_kernel=(1, 3, 3, 1), use_bias=True):
        ...
        self.weight = get_weight(self, (kernel, kernel, fmaps, fmaps))
    ...
    def forward(self, x):
        ...
        if self.up:
            x = upsample_conv_2d(x, self.weight, data_format='NHWC', k=self.resample_kernel)
        elif self.down:
            x = conv_downsample_2d(x, self.weight, data_format='NHWC', k=self.resample_kernel)
        else:
...
def upsample_conv_2d(x, w, k=None, factor=2, gain=1, data_format='NHWC'):
    ...
    stride = [factor, factor]
    x = F.conv_transpose2d(x, w, stride=stride, padding=0)
    return _simple_upfirdn_2d(x, k, pad0=(p + 1) // 2 + factor - 1, pad1=p // 2 + 1, data_format=data_format)
...
def upfirdn_2d(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
    ...
    x = F.pad(x, (0, 0, 0, upy - 1, 0, 0, 0, upx - 1))
    x = x.reshape(-1, inH * upy, inW * upx, minorDim)

    # Pad (crop if negative).
    x = F.pad(x, (0, 0, max(pady0, 0), max(pady1, 0), max(padx0, 0), max(padx1, 0)))
    ...
    w = k.flip([0, 1]).view(1, 1, kernelH, kernelW)
...
def upsample_2d(x, k=None, factor=2, gain=1, data_format='NHWC'):
    ...
    k = _setup_kernel(k) * (gain * (factor ** 2))
    p = k.shape[0] - factor
    return _simple_upfirdn_2d(x, k, up=factor, pad0=(p + 1) // 2 + factor - 1, pad1=p // 2, data_format=data_format)
...
def downsample_2d(x, k=None, factor=2, gain=1, data_format='NHWC'):
    ...
    k = _setup_kernel(k) * gain
    p = k.shape[0] - factor
    return _simple_upfirdn_2d(x, k, down=factor, pad0=(p + 1) // 2, pad1=p // 2, data_format=data_format)
